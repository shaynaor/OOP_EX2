...
class ConvertGpsToPixel {
	...
	@Test
	/**
	 * This test generates random map width and height as well random map boarders represented
	 ...
	 * if the gps point moved in the lat or lon more then 0.5 the test will fail.
	...
	void GpsToPixelAndBack() {
		...
		for(int i = 0 ; i<10000 ;i++) {
			...
			height = ThreadLocalRandom.current().nextInt(1, 5000 + 1);
			width = ThreadLocalRandom.current().nextInt(1, 5000 + 1);
			//====Setting map =====
			...
			if(Math.abs((gps.x()-gpsAfter.x()))>0.5 || Math.abs((gps.y()-gpsAfter.y()))>0.5) {
